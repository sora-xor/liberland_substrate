...
fn single_proposal_should_work_with_delegation() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			tally(r),
			Tally { ayes: 30, nays: 0, turnout: 30, aye_voters: 20000, nay_voters: 0 }
		);
		...
		assert_eq!(
			tally(r),
			Tally { ayes: 60, nays: 0, turnout: 60, aye_voters: 30000, nay_voters: 0 }
		);
		...
		assert_eq!(
			tally(r),
			Tally { ayes: 50, nays: 0, turnout: 50, aye_voters: 30000, nay_voters: 0 }
		);
		...
		assert_eq!(
			tally(r),
			Tally { ayes: 20, nays: 0, turnout: 20, aye_voters: 20000, nay_voters: 0 }
		);
...
fn cyclic_delegation_should_unwind() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			tally(r),
			Tally { ayes: 30, nays: 30, turnout: 60, aye_voters: 10000, nay_voters: 20000 }
		);
	});
}
...
fn single_proposal_should_work_with_vote_and_delegation() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			tally(r),
			Tally { ayes: 10, nays: 20, turnout: 30, aye_voters: 10000, nay_voters: 10000 }
		);
...
fn single_proposal_should_work_with_undelegation() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			tally(r),
			Tally { ayes: 10, nays: 0, turnout: 10, aye_voters: 10000, nay_voters: 0 }
		);
	});
}
